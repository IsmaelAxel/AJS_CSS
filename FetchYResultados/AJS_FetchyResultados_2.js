...
if (!funConsultarAlaApi.exists) {
    ...
    var ocurrenciasFilter = (funConsultarAlaApi.toString().match(/alert\(/g) || []).length;

        if (ocurrenciasFilter !== 3) {
        seguirValidando = false;
        arrojarError('Dentro de la función consultarAlaApi() debes usar 2 alert para mostrar el valor del título (strCategory) e imagen (strCategoryThumb) en cada uno');
        }
}
